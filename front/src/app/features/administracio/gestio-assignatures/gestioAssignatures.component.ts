...
@Component({
  selector: 'admin-llista-assignatures',
  imports: [CommonModule, FormsModule, SidebarComponent],
  templateUrl: './gestioAssignatures.component.html',
  styleUrl: './gestioAssignatures.component.css',
})
export class GestioAssignatures implements OnInit {
  ...
  get isLoading() {
    return this.assignaturesManager.isLoading();
  }
  get error() {
    return this.assignaturesManager.error();
  }
  ...
  crearAssignatura(): void {
    ...
    this.assignaturesManager
      .afegirAssignatura(requestData)
      .then(() => {
  ...
  eliminarAssignatura(id: number): void {
    if (confirm("Estàs segur d'esborrar aquesta assignatura?")) {
      this.assignaturesManager.esborrarAssignatura(id).catch((err) => {
